"""
Fourier Neural Operator for 2D problems with minimal essential improvements.
...
def train_fno2d_pro(model, train_loader, test_loader, ntrain, ntest, epochs, learning_rate, device, batch_size, s):
    optimizer = Adam(model.parameters(), lr=learning_rate)  # No weight decay
    criterion = nn.MSELoss()
    myloss = LpLoss(size_average=False)
    
    # Create CSV file with fixed name
    csv_filename = 'training_metrics_2d_pro.csv'
    ...
    for ep in range(epochs):
        ...
        if test_l2 < best_test_loss:
            ...
            model_save_path = 'best_model_fno2d_pro.pth'
            torch.save(model.state_dict(), model_save_path)
            with open(csv_filename, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Best model saved at epoch', ep, 'with test_l2', f'{test_l2:.6e}'])
...
if __name__ == "__main__":
    ...
    config_filename = 'model_config_2d_pro.csv'
    ...
    train_fno2d_pro(model, train_loader, test_loader, ntrain, ntest, epochs, learning_rate, device, batch_size, s) 